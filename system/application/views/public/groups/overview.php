<?= $this->load->view('_template/public_header.html'); ?>
  ...
  <script type="text/javascript">
      ...
      function removeItem(group_id) {
        ...
        if(bestaetigung) {
          document.location.href = '<?=base_url();?>group/remove/'+group_id;
        }
      } 
        ...
        if ( $(".addButtonTop").length > 0 ) {
          ...
          //////////////typeahead///////
    var sourceArr;
    $.ajax({url:'<?=base_url();?>search/group',success:function(result){
...

{
  ...
  // outputs, e.g., "my_dataset"
       ...
       // alert("Selected: "+datum.id + "name " + datum.value );
        document.location.href = '<?=base_url();?>group/show/' + datum.id;
...
<div id="the-basics">
      ...
      <input class="typeahead" type="text" placeholder="Etikett finden...">
      </div>
      
      <div id="scroller" class="scrollerNav">
        <?php if ($this->session->userdata('role') == "admin" || $this->session->userdata('role') == "edit"): ?>
      <div class="addButtonTop"><a class="newWindow" href="<?=base_url();?>group/showAdd"><span class="greenFont">+</span> Etikett hinzufügen</div></a>
      <?php endif; ?>
        
        
            <ul id="navLeft">
          <?php foreach ($nav as $name) :?>
            <li onclick="document.location.href = '<?=base_url();?>group/show/<?= $name->id; ?>'"><p><?= $name->name; ?></p><span class="navArrow"></span>
    ...
    <div id="rightBox">
      ...
      <div id="content">
        ...
        <div id="left">
          <form class="data" action="<?=base_url();?>group/save/<? if(isset($formular->id)) { echo $formular->id; } ?>" method="post">
            ...
            <div class="clear"></div>
            <?php endif;?>
            <label class="user" for="groupname">Etikettname:</label>
            <input type="text" id="groupname" name="groupname" disabled="disabled" value="<? if(isset($formular->name)) { echo $formular->name; } ?>">
            <div class="clear"></div> 
          ...
          <div class="greyBox" onclick="removeItem('<? if(isset($formular->id)) { echo $formular->id; } ?>')">
           <img src="<?=base_url();?>css/images/deleteIcon.png" alt="Löschen" /><p>Etikett löschen</p>
           <div class="clear"></div>
          </div>
          <?php endif;?>
        </div>
